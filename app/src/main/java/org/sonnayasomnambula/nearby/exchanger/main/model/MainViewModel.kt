...
import org.sonnayasomnambula.nearby.exchanger.common.__func__
...
data class MainScreenState (
    ...
    val hasActiveTransfer: Boolean
        get() = incoming.hasData || outgoing.hasData
}
...
sealed interface MainScreenEvent {
    ...
    data object SendCustomClicked : MainScreenEvent
...
sealed interface MainScreenEffect {
    ...
    data object RequestFileAccess : MainScreenEffect
    ...
    data object PickCustom : MainScreenEffect
}
...
class MainViewModel(
    ...
    private sealed interface PendingAction {
        @Parcelize
        data class StartService(val role: Role) : PendingAction, Parcelable
        @Parcelize
        data object AddSaveDirectory : PendingAction, Parcelable
        @Parcelize
        data object SendFile : PendingAction, Parcelable
        @Parcelize
        data object SendDirectory : PendingAction, Parcelable
        @Parcelize
        data object SendCustom : PendingAction, Parcelable
    }

    private var pendingAction by Keeper.Tag<PendingAction>("pendingAction", keeper)
    ...
    fun onScreenEvent(event: MainScreenEvent) {
        ...
        when (event) {
            ...
            is MainScreenEvent.SendCustomClicked -> onSendCustomClicked()
    ...
    private fun onPermissionResult(granted: Boolean) {
        viewModelScope.launch {
            ...
            if (action is PendingAction.SendCustom) {
                if (granted) {
                    pendingAction = action
                    _activityEffects.send(MainScreenEffect.PickCustom)
                }
            }
        }
    }
    ...
    fun picked(files: List<TransferEngine.File>) {
        val action = requireNotNull(pendingAction)
        pendingAction = null
    ...
    private fun onSendCustomClicked() {
        viewModelScope.launch {
            pendingAction = PendingAction.SendCustom
            if (permissionPolicy.hasLegacyStorageAccess()) {
                _activityEffects.send(RequestPermissions(permissionPolicy.storageAccessPermissions()))
            } else {
                _activityEffects.send(RequestFileAccess)
            }
        }
    }
