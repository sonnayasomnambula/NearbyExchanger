...
import org.sonnayasomnambula.nearby.exchanger.main.MainActivity
...
class ExchangeService : Service() {
    ...
    companion object {
        ...
        fun start(role: Role, context: Context) {
            ...
            intent.putExtra(Role::class.simpleName, role)
        ...
        when (action) {
            ACTION_START -> {
                val role = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    intent.getSerializableExtra(Role::class.simpleName, Role::class.java)
                } else {
                    @Suppress("DEPRECATION")
                    intent.getSerializableExtra(Role::class.simpleName) as Role
                }
